function App() {
  // Instant paint: hydrate from the last successful load (a localStorage
  // snapshot saved after every clean fetch) so repeat visits render the
  ...
  React.useEffect(function () {
    Promise.all([storageGet("voxel-catalog"), storageGet("voxel-settings"), storageGet("voxel-content")]).then(function (results) {
      ...
      if (!catalogResult.ok || !settingsResult.ok || !contentResult.ok) {
        // A failed read means "unknown state", NOT "empty shop". Seeding
        // defaults over a temporarily-unreadable database is exactly how
        // a real catalog gets erased, so on any read failure we touch
        // nothing. If the visitor already has the last good shop on
        // screen (painted instantly from the cache at mount), keep it
        // instead of a scary error screen — the network will very likely
        // be back on the next visit. With no cache there is nothing to
        // show, so fail loudly with the retry screen.
        if (cachedCatalog) {
          setLoading(false);
        } else {
          setLoadError(true);
